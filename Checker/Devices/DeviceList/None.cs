            ...
            switch (step.Command)
            {
                case DeviceCommands.CalculateCoefficient:
                {
                    ...
                    try
                    {
                        ...
                        var result =
                            $"Коэффициент равен {string.Format("{0:0.000}", actualCoefficient)} В/мкА \tНижний предел  {lowerLimit} В/мкА \tВерхний предел {upperLimit} В/мкА";
                ...
                case DeviceCommands.CheckResistancesDifference:
                {
                    var expectedDifference = double.Parse(step.Argument);
                    var valuesDictionary = GetValuesDictionary();
                    var resistances = valuesDictionary.Keys
                        .Where(k => k.Contains("R"))
                        .Select(k => valuesDictionary[k])
                        .ToList();
                    var dR = resistances.Max() - resistances.Min();
                    var result = $"Значения сопротивлений отличаются на {dR}";
                    return dR >= expectedDifference ? DeviceResult.ResultError(result) : DeviceResult.ResultOk(result);
                }
                default:
...
/*            catch (KeyNotFoundException)
        ...
        private DeviceResult GetCoefficient_UCAT(Steps.Step step)
        {
            var value = double.Parse(step.Argument, NumberStyles.Float);
            var lowerLimit = step.LowerLimit;
            var upperLimit = step.UpperLimit;
